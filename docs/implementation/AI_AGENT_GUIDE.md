...
## Common Mistakes
- *Mistake:* Using a generic `<div>` for a clickable card. *Correction:* Use an `<a>` or `<button>` wrapper for accessibility.
- *Mistake:* Attaching animation logic directly to a `scroll` event. *Correction:* Use `IntersectionObserver`.
- *Mistake:* Using arbitrary dark grays for backgrounds. *Correction:* Use `var(--primary-800)` or `bg-primary-800`.
